  ...
  return (
    <main>
      ...
      <section className="movies">
        {movies.map(movie => {
          return (
            <div className='movie' data-testid='movieItem' key={movie.id}>
              <h3>{movie.title}</h3>
              <img className='poster' data-testid="toDetails" onClick={() => handleClick(movie.id)} src={movie.poster} alt={movie.title}/>
            </div>
          );
        })}
      </section>
